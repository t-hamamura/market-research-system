...
**📋 重要な発見事項**（必須：具体的な3-5点）:
1. **市場機会**: [具体的な市場規模・成長率データ][1]
2. **競合優位性**: [差別化ポイントの詳細][2]  
3. **顧客ニーズ**: [ターゲット顧客の具体的ニーズ][3]
4. **技術的優位性**: [技術面での競争優位][4]
5. **収益可能性**: [収益モデルの実現可能性][5]

| 発見事項 | 重要度 | 事業への影響 | エビデンス |
|----------|--------|-------------|------------|
| 発見1 | 高 | 具体的影響 | [1] |
| 発見2 | 中 | 具体的影響 | [2] |
...
| 評価項目 | スコア | 根拠 | 参照 |
|----------|--------|------|------|
| 市場規模 | High/Medium/Low | 具体的数値 | [1] |
| 競合優位性 | High/Medium/Low | 差別化ポイント | [2] |
| 実行難易度 | High/Medium/Low | 課題と対策 | [3] |

**⚡ 推奨アクション**（必須：優先順位付き）:
1. **短期（3ヶ月）**: [具体的なアクション][4]
2. **中期（6-12ヶ月）**: [戦略的施策][5]
3. **長期（1-3年）**: [ビジョン実現][6]
...
**脚注システム**:
- 本文中に [1], [2], [3] 等の番号を挿入
- 数値データ、重要な主張、戦略提言には必ず脚注を付与
- レポート末尾に参考情報セクションを設置

**参考情報セクション**:
[1] [業界レポート名](URL) - データの概要説明
[2] [競合分析情報](URL) - 情報の内容説明
[3] [市場調査データ](URL) - 使用した具体的データ

**重要**: 重要な発見事項と推奨アクションは必ず詳細に記載し、それぞれに適切なエビデンス（脚注）を付けてください。
